...
namespace Digi.Utils{
    class Log{
        private const string MOD_NAME = "PlanetDynamics";
        ...
        public static void Error(string msg){
            ...
            try{
                MyAPIGateway.Utilities.ShowNotification(MOD_NAME+" error logged.", 1000, MyFontEnum.Red);
            } catch(Exception e){
        ...
        private static void Write(string msg){
            if(writer == null){
                if(MyAPIGateway.Utilities == null)
                    throw new Exception("API not initialized but got a log message: " + msg);
